<script lang="ts">
	/**
	 * TrajectoryPanel - Chronological list of model calls and tool calls for a loop.
	 *
	 * Fetches trajectory via trajectoryStore and renders each entry through
	 * TrajectoryEntryCard (which delegates to specialized components for
	 * spawn_agent, create_tool, and decompose_task).
	 */
	...
	const modelCallCount = $derived(entries.filter((e) => e.type === 'model_call').length);
	const toolCallCount = $derived(entries.filter((e) => e.type === 'tool_call').length);
...
<div class="trajectory-panel" class:compact>
	<div class="panel-header">
		<div class="header-title">
			<Icon name="clock" size={compact ? 14 : 16} />
	...
	<div class="panel-content">
		...
		{:else if entries.length === 0}
			<div class="empty-state">
				<Icon name="clock" size={compact ? 18 : 24} />
		...
		{:else}
			<ol class="entry-list" aria-label="Trajectory entries">
