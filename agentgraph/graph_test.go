...
func (m *mockEntityManager) UpdateEntityWithTriples(_ context.Context, entity *gtypes.EntityState, add []message.Triple, _ []string) (*gtypes.EntityState, error) {
	...
	// Merge the new triples into a copy stored by entity ID.
...
func TestHelper_RecordLoopCreated(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// All triples must carry the Semsage source.
			for _, triple := range entity.Triples {
				if triple.Source != agentgraph.SourceSemsage {
					t.Errorf("triple %q has source %q, want %q", triple.Predicate, triple.Source, agentgraph.SourceSemsage)
				}
			}
		})
	}
...
func TestHelper_RecordLoopStatus(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Verify that an UpdateEntityWithTriples call occurred with the correct status triple.
			if len(em.updated) == 0 {
