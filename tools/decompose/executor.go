...
// (e.g. unexpected JSON marshalling panics) that the agentic-tools dispatcher
// should treat as fatal — none arise in this passthrough implementation.
...
func parseNodes(raw any) (TaskDAG, error) {
	...
	for i, item := range slice {
		...
		if raw, exists := m["depends_on"]; exists && raw != nil {
			deps, ok := raw.([]any)
			if !ok {
				return TaskDAG{}, fmt.Errorf("nodes[%d]: depends_on must be an array, got %T", i, raw)
			}
